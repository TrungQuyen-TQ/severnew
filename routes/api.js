...
router.get('/chef/pending-meals', authenticateToken, async (req, res) => {
    ...
    if (req.user.role !== 'chef') {
        return res.status(403).json({ error: 'Truy cập bị từ chối. Chỉ dành cho Đầu bếp.' });
    }
...
router.put('/chef/serve-order/:orderId', authenticateToken, async (req, res) => {
    // Thêm check role 'chef' nếu bạn có hàm checkRole riêng
    if (req.user.role !== 'chef') {
        return res.status(403).json({ error: 'Truy cập bị từ chối. Chỉ dành cho Đầu bếp.' });
    }
    ...
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: 'Không tìm thấy đơn hàng đang chờ hoặc đơn hàng đã được phục vụ.' });
        }
        
        res.json({ success: true, message: `Đơn hàng #${orderId} đã được chuyển sang trạng thái SERVED.` });
    } catch (error) {
